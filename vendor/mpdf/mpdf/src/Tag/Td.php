...
class Td extends Tag
{
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ($this->mpdf->simpleTables && $this->mpdf->row == 0 && $this->mpdf->col == "no") {
		...
		if ($this->mpdf->table_border_attr_set && $table['border_details']) {
			...
			} elseif ($this->mpdf->simpleTables && $this->mpdf->row == 0 && $this->mpdf->col == "no") {
		...
		if ($this->mpdf->simpleTables && $this->mpdf->row == 0 && $this->mpdf->col == "no" && !$table['borders_separate'] && $table['simple']['border']) {
